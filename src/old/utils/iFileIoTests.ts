...
export function iFileIoTests(fileIo:IFileIo | IFileIoSync, mode: 'IFileIo' | 'IFileIoSync') {
    ...
    test('lists files in a directory', async () => {
        ...
        expect(files).toEqual([
            { basename: 'test2.txt', dirname: TMP_DIR, uri: `${TMP_DIR}/test2.txt` },
            { basename: 'test1.txt', dirname: TMP_DIR, uri: `${TMP_DIR}/test1.txt` },
        ]);
    });
    ...
    test('lists files with recursion', async () => {
        ...
        expect(files).toEqual([
            { basename: 'test1.txt', dirname: TMP_DIR, uri: `${TMP_DIR}/test1.txt` },
            { basename: 'test2.txt', dirname: subDir, uri: `${subDir}/test2.txt` },
        ]);
    });
